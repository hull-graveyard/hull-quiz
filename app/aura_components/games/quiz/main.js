 ...
 * @param {String} autoAdvance boolean: Advance to next question on click on an Answer (default:false)
 * @param {String} autoSubmit boolean: Skip final screen and auto submit quiz (default:false)
 * @param {String} loginFirst boolean: Ask the user to login before playing (default:true)
 * @param {String} highlightAnswers boolean: Highlight the currently selected answer with an 'active' class (default:false)
 * @param {String} autoStart boolean: Skip the intro screen (default:false)
 * @param {String} sampleQuestions number: Choose n random questions among the total available (default: Show all questions)
 * @param {String} quizTimer number: Add a global timer to the quiz (default:none)
...
Hull.component({
  ...
  answer: function (event, action) {
    ...
    this.track('hull.quiz.progress', {
      quizId: this.id,
      questionRef: action.data.questionRef,
      answerRef: action.data.answerRef,
      questionIndex: this.currentQuestionIndex,
      questionsCount: this.data.quiz.get('questions')
        .length
    });
  },
  // Rendering
  ...
  beforeRender: function (data) {
    ...
    data.readyToPlay = (data.loggedIn || !this.options.loginFirst)
    data.current = this.currentQuestionIndex+1;
    data.previous = !this.currentQuestionIndex;
    data.next = !this.isLastQuestion()
  ...
  isLastQuestion: function () {
    return this.currentQuestionIndex != this.getQuestions().length;
  },
